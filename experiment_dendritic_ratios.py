...
def find_valid_combinations(target_params=16000, in_dim=784, tolerance=0.05, n_experiments=9):
    ...
    if n_experiments == 1:
        ...
        all_ratios = [1/100, 1/50, 1/25, 1/10, 1/8, 1/5, 1/4, 1/3, 1/2]
    ...
    print(f"Testing {n_experiments} experiments with ratios: {[f'{r:.4f}' for r in dendrite_input_ratios]}")
    ...
    for ratio in dendrite_input_ratios:
        ...
        actual_params = calculate_total_model_params(n_dendrites, n_dendrite_inputs, n_classes)
...
def create_model(n_dendrite_inputs, n_dendrites, n_classes, in_dim, synaptic_resampling=False):
...
def run_experiment(
    ...
    print(f"Finding valid combinations for {target_params} total model parameters (dendritic + linear layers)...")
    combinations = find_valid_combinations(target_params, in_dim, n_experiments=n_experiments)
    ...
    for i, combo in enumerate(combinations[:3]):  # Show first 3
        print(f"  {i+1}: n_dendrite_inputs={combo['n_dendrite_inputs']}, "
    ...
    for combo in combinations:
        for synaptic_resampling in synaptic_options:
            ...
            try:
                ...
                # Create optimizer
                optimizer = Adam(model.params(), criterion, lr=lr, weight_decay=weight_decay)
                ...
                final_test_acc = result_data['test_accuracy'][-1]  # Changed from 'test_accuracies'
                final_train_acc = result_data['train_accuracy'][-1]  # Changed from 'train_accuracies'
...
def print_best_ratios(results):
    ...
    print(f"  • n_dendrite_inputs/in_dim ratio: {best_result['dendrite_input_ratio']:.4f}")
    print(f"    (n_dendrite_inputs = {best_result['n_dendrite_inputs']} out of {28*28} inputs)")
    print(f"  • n_dendrites/in_dim ratio: {best_result['dendrite_ratio']:.4f}")
    print(f"    (n_dendrites = {best_result['n_dendrites']})")
    print(f"  • Synaptic resampling: {'ON' if best_result['synaptic_resampling'] else 'OFF'}")
    print()
...
def plot_results(results):
    ...
    fig.suptitle('Dendritic Parameter Analysis - Accuracy vs Architecture', fontsize=16, fontweight='bold')
    
    # Plot 1: n_dendrites vs n_dendrite_inputs colored by accuracy
    scatter1 = axes[0, 0].scatter(n_dendrite_inputs, n_dendrites, c=accuracies, 
    ...
    axes[0, 1].scatter(n_dendrite_inputs, accuracies, c=colors2, s=100, alpha=0.7, edgecolors='black')
    ...
    axes[1, 0].scatter(n_dendrites, accuracies, c=colors2, s=100, alpha=0.7, edgecolors='black')
    ...
    axes[1, 1].scatter(dendrite_input_ratios, accuracies, c=colors2, s=100, alpha=0.7, 
                       edgecolors='black', label='Input Ratio')
    axes[1, 1].scatter(dendrite_ratios, accuracies, c=colors2, s=100, alpha=0.7, 
                       edgecolors='black', marker='^', label='Dendrite Ratio')
    ...
    if len(successful_results) > 1:
        ...
        best_result = max(successful_results, key=lambda x: x['final_test_accuracy'])
        print(f"  • Best configuration uses {best_result['n_dendrite_inputs']} inputs per dendrite")
        print(f"  • Best configuration has {best_result['n_dendrites']} total dendrites")
        print(f"  • This represents {best_result['dendrite_input_ratio']*100:.1f}% input connectivity")
...
def save_results(results, filename=None):
    ...
    if filename is None:
        filename = f'dendritic_experiment_{datetime.now().strftime("%Y%m%d_%H%M%S")}.json'
