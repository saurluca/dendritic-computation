...
def optimize_for_budget(max_params=8000, dataset="fashion-mnist", n_trials=50):
    ...
    if max_params <= 2000:
        ...
        param_distributions = {
            ...
            'soma_enabled': [True, False],
            'architecture': ['dendritic_simple', 'dendritic_with_hidden', 'soma_disabled'],
            'synaptic_resampling': [True, False],
...
def compare_architectures(max_params=8000, dataset="fashion-mnist"):
    ...
    for arch_name, arch_desc in architectures:
        ...
        if arch_name == 'dendritic_simple':
            ...
            param_grid = {
                'n_neurons': [8, 10, 12],
                'n_dendrites': [15, 20, 25, 30],  # More dendrites since they're direct outputs
        ...
        if searcher.results:
            ...
            print(f"✅ Best accuracy: {best.test_accuracy:.3f} ({best.total_params} params)")
        else:
...
def find_pareto_optimal(dataset="fashion-mnist", max_budget=10000):
    ...
    for result in pareto_results:
        efficiency = result['accuracy'] / (result['actual_params'] / 1000)  # Accuracy per 1K params
        print(f"Budget: {result['budget']:5d} | "
...
def main():
    ...
    if quick_result:
        print(f"✅ Quick prototype found: {quick_result.test_accuracy:.3f} accuracy with {quick_result.total_params} params")
    
    # Example 2: Budget optimization
    print("\n\n2️⃣  BUDGET OPTIMIZATION")
    budget_result = optimize_for_budget(max_params=8000, dataset="fashion-mnist", n_trials=25)
    
    if budget_result:
        print(f"✅ Best model: {budget_result.test_accuracy:.3f} accuracy with {budget_result.total_params} params")
    ...
    if arch_results:
        ...
        print(f"✅ Best architecture: {best_arch['architecture']} with {best_arch['test_accuracy']:.3f} accuracy")
