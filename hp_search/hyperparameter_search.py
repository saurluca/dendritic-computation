...
class HyperparameterSearcher:
    ...
    def _load_data(self):
        ...
        if self.verbose:
            ...
            print(
                f"Parameter budget: {self.min_params}-{self.max_params} ({self.min_param_ratio * 100:.0f}%-100%)"
            )
    ...
    def calculate_dendritic_params(
        ...
        if soma_enabled:
            ...
            dendrite_params = (
                n_dendrite_inputs * n_soma_connections + n_soma_connections
            )  # weights + biases
            ...
            dendrite_params = (
                n_dendrite_inputs * n_dendrites + n_dendrites
            )  # weights + biases
    ...
    def estimate_params(
        ...
        if architecture == "dendritic_simple":
            if soma_enabled:
                dendrite_params = (
                    n_dendrite_inputs * n_dendrites * n_neurons
    ...
    def is_valid_config(self, params: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if total_params < self.min_params:
                if self.verbose:
                    print(
                        f"Config uses too few parameters: {total_params} < {self.min_params} ({self.min_param_ratio * 100:.0f}% of budget)"
                    )
                return False
    ...
    def train_and_evaluate(self, params: Dict[str, Any]) -> Optional[SearchResult]:
        ...
        try:
            ...
            optimizer = Adam(
                model.params(), criterion, lr=self.lr, weight_decay=self.weight_decay
            )
    ...
    def grid_search(
        self, param_grid: Dict[str, List[Any]], max_trials: Optional[int] = None
    ) -> List[SearchResult]:
        """
        ...
        if self.verbose:
            ...
            print(
                f"Parameter budget: {self.min_params}-{self.max_params} parameters ({self.min_param_ratio * 100:.0f}%-100%)"
            )
    ...
    def random_search(
        self, param_distributions: Dict[str, Any], n_trials: int = 50
    ) -> List[SearchResult]:
        """
